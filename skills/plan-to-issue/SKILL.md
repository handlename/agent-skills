...
- **Big-picture diagram is mandatory**: the issue description MUST include a diagram that lets a reader grasp the whole at a glance — the problem/system the issue targets (e.g. affected components and their relationships, current vs. changed data flow, or the state transition being fixed). Author it as **mermaid** — the mermaid source is the **canonical record of the diagram's structure**. Choose the diagram type that best conveys the structure (`flowchart`, `sequenceDiagram`, `erDiagram`, `stateDiagram-v2`, etc.). **Author the mermaid via a subagent invoking the `draw-mermaid` skill** (input: the diagram intent; output: a syntax-validated mermaid source, or `FAILED: <reason>`) so it is guaranteed to parse before it is shown or handed to `mermaid-to-svg`. It is for human comprehension, not implementation detail; keep it high-level. In existing-issue mode there is no new description — place the diagram near the top of the plan comment instead.
- **Diagram is posted as an SVG image; mermaid is preserved in a collapsed `<details>` block**: the diagram is displayed as a **free-layout SVG image**, not a rendered mermaid block. Produce the image in two steps, each a **subagent**: first `mermaid-to-svg` (input: the mermaid source; output: an SVG file path, or `FAILED: <reason>`), then `github-attachment-upload` (input: that SVG path + target repository; output: an attachment URL, or `FAILED: <reason>`). The main session then embeds `![big picture](<url>)` itself, immediately followed by the mermaid source inside `<details><summary>…</summary>` with a ` ```mermaid ` fence — collapsed by default so it stays out of the way, one click away as the canonical structure record, and it renders as a diagram when expanded (a bonus fallback for contexts that cannot show the image). **Never wrap the mermaid in an HTML comment** (`<!-- -->`): mermaid arrows (`-->`) contain `--`, which terminates the comment early and leaks the rest of the source as visible text.
  - **Update flow**: to change the diagram, edit the mermaid in the details block, re-run `mermaid-to-svg` to redraw the SVG, re-invoke `github-attachment-upload` with the new file, and replace the image URL. The SVG layout is throwaway and is redrawn from scratch each time; only the mermaid persists.
  - **Fallback**: if either subagent returns `FAILED` (`mermaid-to-svg` could not draw the SVG, or `github-attachment-upload` hit e.g. GitHub session not logged in / npx cannot fetch @playwright/cli), post a visible ` ```mermaid ` code block instead — the pre-SVG behavior — and report the fallback and its reason to the user. Never let the image block the posting.
...
4. **Draft the description** *(new-issue mode only)* — Overview (what & why) + the **big-picture diagram authored as mermaid via a subagent invoking `draw-mermaid`** (the draft shows the syntax-validated mermaid source; it becomes an SVG image in step 8) + Scope (in/out) + PR-granularity checklist with `TASK-` IDs and dependency notes.
...
8. **Generate & upload the diagram image** — spawn a subagent invoking `mermaid-to-svg` with the mermaid source to draw the SVG (→ file path), then spawn a subagent invoking `github-attachment-upload` with that file path and the target repository (→ attachment URL). On success (URL returned): replace the draft's mermaid block with `![big picture](<url>)` followed by the mermaid source in a collapsed `<details>` block. On `FAILED: <reason>` from either subagent: keep the visible mermaid code block and report the fallback and reason to the user.
...
## 全体像 / Big picture
{an SVG image drawn by mermaid-to-svg and uploaded via github-attachment-upload; the mermaid source stays below in a collapsed details block as the canonical structure record — NOT an HTML comment (`-->` arrows would terminate it). On FAILED fallback: replace the image line with a visible mermaid code block instead.}
...
- [ ] Diagram posted as an SVG image (drawn by a subagent invoking `mermaid-to-svg`, then uploaded by a subagent invoking `github-attachment-upload`, after the authorization gate) with the mermaid source preserved in a collapsed `<details>` block right after the image (never an HTML comment — `-->` terminates it) — or, if either subagent FAILED, posted as a visible mermaid code block with the fallback reported to the user.
